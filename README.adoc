...
ui:
  bundle:
    url: https://github.com/feelpp/antora-ui/releases/download/v0.54.0/ui-bundle.zip
----

NOTE: Update the version in the download URL. Here, we are using version `v0.54.0`.
...
The documentation repository must enable `@feelpp/antora-extensions` to generate its index; see link:SEARCH-README.md[SEARCH-README.md].
...
|Explains that the Antora Lunr extension generates the index during a site build

|`npm run build:full`
|Build the UI bundle and the UI documentation site
